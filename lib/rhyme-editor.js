...
export default class RhymeEditor {
  static openTab (rhymingDictionary, rhymeTypes) {
    const tabName = this.getTabName(rhymingDictionary.target, rhymeTypes)
    const opts = {split: 'right', activatePane: false}
    var rhymeEditor
    return atom.workspace.open(tabName, opts).then(editor => {
      rhymeEditor = new RhymeEditor(editor, rhymingDictionary, rhymeTypes)
      return rhymeEditor
    }).then((rhymeEditor) => {
